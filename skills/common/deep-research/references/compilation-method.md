...
- The user has several research sources (AI-assisted reports, Word/PDF whitepapers, URLs, Markdown drafts) and wants them compiled into one coherent document.
...
For citation extraction, regex-scan each section's `content_md` for `\[(\d+(?:\s*,\s*\d+)*)\]` patterns. Some generated Word documents render citations as small superscript text without bookmarks; when a superscript run contains only digits, treat each digit as a citation local_num.
...
`_render_runs` walks markdown-style inline tokens (`**bold**`, `*italic*`, `` `code` ``, `[link](https://example.com)`) and emits a run for each. Citations are stripped first so the markdown-link regex cannot eat a `[N]` by mistake.
