...
description: Asset preprocessing for HyperFrames compositions — text-to-speech narration (Kokoro), audio/video transcription (Whisper), and background removal for.
---
...
```bash
npx hyperframes tts "The meeting starts at nine" --voice ef_dora --output voice-dora.wav
npx hyperframes tts "Today is good weather" --voice jf_alpha --output voice-alpha.wav
```
