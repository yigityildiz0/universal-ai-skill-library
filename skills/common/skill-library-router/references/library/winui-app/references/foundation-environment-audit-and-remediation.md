...
1. Use the setup-and-scaffold flow in [../INSTRUCTIONS.md](../INSTRUCTIONS.md) for environment readiness, remediation, and initial verification.
...
- The setup-and-scaffold flow in `INSTRUCTIONS.md` over ad hoc manual checks or duplicated setup instructions in this reference.
- A short manual audit only when the user wants a non-mutating readiness check.

## Avoid

- Rewriting or paraphrasing the bundled setup workflow here when `INSTRUCTIONS.md` already covers the user's goal.
...
- Missing any required WinUI prerequisite:
  - Use the setup-and-scaffold flow in `INSTRUCTIONS.md` after confirmation when the request is audit-only.
...
- Was the setup-and-scaffold flow in `INSTRUCTIONS.md` used before advice was given?
