...
```bash
opencode mcp add --transport http nimble https://mcp.nimbleway.com/mcp \
  --header "Authorization: Bearer ${NIMBLE_API_KEY}"
```

> **Restart opencode after running this** — MCP servers added mid-session aren't available until the next launch.
