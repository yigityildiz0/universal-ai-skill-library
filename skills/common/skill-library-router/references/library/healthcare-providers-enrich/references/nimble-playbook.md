...
How to run Nimble CLI commands in opencode. Read this before executing any commands.
...
- **No `&` + `wait` parallelism.** It breaks in opencode. Instead, make **multiple
...
| `opencode mcp list 2>/dev/null \| grep -q "nimble"` (or first `mcp__plugin_nimble_nimble__*` call succeeds) | Plugin MCP is connected | `mcp__plugin_nimble_nimble__*` tools |
...
>
> **Any the assistant product (opencode, the assistant Cowork, claude.ai) — recommended:**
> ```
...
The value is always `skill-` followed by the exact INSTRUCTIONS.md `name` field
...
Each skill provides its own keyword/weight table in INSTRUCTIONS.md — the pattern here is
...
Make **multiple Bash tool calls in a single response**. opencode runs them in
...
Skills define their own P1/P2/P3 signal types in their INSTRUCTIONS.md. The verification
