...
Use `get_design_context` → (optionally `get_metadata` for large nodes) → `get_screenshot`, and keep project rules from `INSTRUCTIONS.md` in mind when applying the generated output.
