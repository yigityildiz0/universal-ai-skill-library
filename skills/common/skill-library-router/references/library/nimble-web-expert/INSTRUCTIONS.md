...
description: Get web data now — fast, incremental, immediately responsive to what the user needs. The only way the assistant can access live websites. USE FOR.
license: MIT
allowed-tools:
- Bash(nimble:*)
- Bash(mkdir:*)
...
opencode mcp list 2>/dev/null | grep -q "nimble" && echo "MCP: ok"  # plugin MCP
```
...
- **Neither** → load `rules/setup.md` for the environment-aware install flow. Any the assistant product (Code, Cowork, claude.ai) → `/plugin install nimble`. Codex or other terminal-only agents → `npm i -g @nimble-way/nimble-cli`. Cursor / VS Code / generic MCP clients → paste the `mcp.json` snippet.
...
1. `~/.config/opencode/skills/nimble-web-expert/learned/examples.json` → `agents[]` array
2. `references/nimble-agents/INSTRUCTIONS.md` → baked-in table (50+ sites)
...
For the full agent list (50+ sites), see `references/nimble-agents/INSTRUCTIONS.md`.
...
| ------------------------------- | -------------------------------------------- | ---------------------------------------------------- |
| Site/domain → check agent first | `nimble agent list` → `nimble agent run`     | `references/nimble-agents/INSTRUCTIONS.md`                  |
| Direct URL                      | `nimble extract`                             | `references/nimble-extract/INSTRUCTIONS.md`                 |
| Search the live web             | `nimble search`                              | `references/nimble-search/INSTRUCTIONS.md`                  |
| Discover URLs on a site         | `nimble map`                                 | `references/nimble-map/INSTRUCTIONS.md`                     |
| Bulk crawl a section            | `nimble crawl run`                           | `references/nimble-crawl/INSTRUCTIONS.md`                   |
| Batch agents (up to 1,000)      | `nimble agent run-batch`                     | `references/nimble-agents/INSTRUCTIONS.md`                  |
| Batch extract (up to 1,000)     | `nimble extract-batch`                       | `references/nimble-extract/INSTRUCTIONS.md`                 |
| Poll tasks / batches / results  | `nimble tasks` / `nimble batches`            | `references/nimble-tasks/INSTRUCTIONS.md`                   |
...
For the full extract waterfall (tiers, flags, browser actions, network capture), see `references/nimble-extract/INSTRUCTIONS.md`.
...
The skill maintains `~/.config/opencode/skills/nimble-web-expert/learned/examples.json`.
...
- **Task agents MUST use `run_in_background=False`.** See [nimble-agent-builder delegation model](../nimble-agent-builder/INSTRUCTIONS.md#delegation-model) for the why.
...
| `references/nimble-agents/INSTRUCTIONS.md`                  | Step 0 lookup — full agent table (50+ sites)                                  |
| `references/nimble-extract/INSTRUCTIONS.md`                 | Extract flags, render tiers, browser actions, network capture, parser schemas |
| `references/nimble-search/INSTRUCTIONS.md`                  | Search flags, all 8 focus modes                                               |
| `references/nimble-map/INSTRUCTIONS.md`                     | Map flags, response structure                                                 |
| `references/nimble-crawl/INSTRUCTIONS.md`                   | Full async crawl workflow                                                     |
| `references/nimble-tasks/INSTRUCTIONS.md`                   | Poll tasks/batches, fetch results — for async, batch, and crawl operations    |
