---
description: One-time setup instructions for Nimble — plugin install (the assistant products), CLI install (terminal agents), or manual MCP config (Cursor/VS Code). Load this only when the quick check in INSTRUCTIONS.md fails.
alwaysApply: false
---
...
| Host | Best path |
|---|---|
| Any the assistant product (opencode, the assistant Cowork, claude.ai) | **Plugin install** — `/plugin install nimble`. Auto-registers MCP as a Connector. OAuth handles auth. **§1 below.** |
...
## 1. Plugin install (the assistant products — recommended)
...
In claude.ai / the assistant Cowork, the connector appears under
...
Saves the API key to `~/.config/opencode/settings.json` so opencode auto-injects it every session — no exports needed.

```bash
python3 -c "
import json, pathlib, subprocess, os

p = pathlib.Path.home() / '.opencode/settings.json'
...
if key and not env.get('NIMBLE_API_KEY'):
    ...
    print('✓ Saved NIMBLE_API_KEY to ~/.config/opencode/settings.json')
print('NIMBLE_API_KEY: ' + ('set' if key else 'MISSING — see API key setup below'))
p.write_text(json.dumps(d, indent=2))
if key:
    print()
    print('✓ Init complete. Restart opencode to activate.')
"
```

**After running init → restart opencode.** The key is auto-injected from that point on.
...
p.write_text(json.dumps(d, indent=2))
print('✓ Saved to ~/.config/opencode/settings.json')
"
```
...
Gives the assistant instant access to the full Nimble documentation — CLI flags, agent schemas, API reference.

**Add with one command:**

```bash
opencode mcp add --transport http nimble-docs https://docs.nimbleway.com/mcp
```
