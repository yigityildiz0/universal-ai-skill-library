...
6. **Preserve raw counts and the original file** - Write QC annotations and filtered outputs to new files; never overwrite the sole raw dataset
7. **Inspect per sample or batch** - Pooled thresholds can erase a valid sample or retain a failed one; compare distributions before applying a shared rule
8. **Record every threshold and removal count** - Treat filtering as an auditable analysis decision, not a cleanup step
9. **Do not overclaim the pipeline** - Ambient RNA, doublets, sample swaps, contamination, and biological validity require additional checks
10. **Require human review** - A qualified analyst must approve filtering before downstream biological interpretation or publication
