...
description: Deploy applications and websites to Vercel. Use when the user requests deployment actions like "deploy my app", "deploy and give me the link", "push this.
metadata:
  author: vercel
  version: 3.0.0
---
...
### No-Auth Fallback - sandboxed environment

**When to use:** Last resort when the CLI can't be installed or authenticated in a sandboxed environment. This requires no authentication — it returns a **Preview URL** (live site) and a **Claim URL** (transfer to your Vercel account).
...
### Sandboxed environments (Codex/chat sandbox)

You likely cannot run `vercel login` or `git push`. Go directly to the **no-auth fallback - sandboxed environment**.
...
### Network Egress Error (sandboxed environment)

If deployment fails due to network restrictions in the sandboxed environment, tell the user:

```
Deployment failed due to network restrictions. To fix this:

1. Open the current app settings/capabilities if available
