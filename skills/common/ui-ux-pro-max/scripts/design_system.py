...
def safe_slug(name: object, fallback: str = "default") -> str:
    """Convert untrusted labels into one safe relative path segment."""
    fallback_slug = re.sub(r"[^a-z0-9_-]+", "-", str(fallback).lower()).strip("-") or "default"
    slug = re.sub(r"[^a-z0-9_-]+", "-", str(name).lower()).strip("-")[:80].strip("-")
    if not slug or slug in WINDOWS_RESERVED_PATH_SEGMENTS:
