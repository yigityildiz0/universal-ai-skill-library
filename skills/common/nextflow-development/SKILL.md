...
| Problem | Fix |
|---------|-----|
| Not installed | Follow the current official Nextflow installation instructions; inspect the downloaded artifact before execution |
| Version unexpectedly old | Create a new tested environment; do not silently self-update a working analysis environment |
...
| Data Type | Pipeline | Tested baseline in this skill | Goal |
|-----------|----------|-------------------------------|------|
| RNA-seq | `rnaseq` | 3.22.2 | Gene expression |
| WGS/WES | `sarek` | 3.7.1 | Variant calling |
| ATAC-seq | `atacseq` | 2.1.2 | Chromatin accessibility |

The bundled samplesheet logic and references were authored against the tested baselines. For a new analysis, inspect the official release notes and schema changes, run the selected release's test profile, and pin the exact passing tag. Do not replace a baseline version string solely because a newer tag exists.
...
| Pipeline | Command |
|----------|---------|
| rnaseq | `nextflow run nf-core/rnaseq -r 3.22.2 -profile test,docker --outdir test_rnaseq` (then test a newer chosen tag separately) |
| sarek | `nextflow run nf-core/sarek -r 3.7.1 -profile test,docker --outdir test_sarek` (then test a newer chosen tag separately) |
...
This skill is provided as a prototype example for active-host-assisted nf-core bioinformatics workflows. The current implementation supports three pipelines (rnaseq, sarek, and atacseq), serving as a foundation that enables the community to expand support to the full set of nf-core pipelines.
