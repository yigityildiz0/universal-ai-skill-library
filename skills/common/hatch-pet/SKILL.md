---
name: hatch-pet
description: "Create, repair, validate, visually QA, and package Codex-compatible animated pets and spritesheets from character art, generated images, concepts, company brands, or references. Use only when the user explicitly asks for the hatch-pet workflow; otherwise prefer the installed Work Pets skills. Turkish triggers: Hatch pet oluştur veya onar, Codex masaüstü evcil hayvanı, pet durumunu kontrol et."
---
...
Before generating base art, row strips, or repair rows, invoke and follow the installed `$imagegen` skill through the current host's skill system. Let the host resolve the skill location; do not assume or open a hardcoded filesystem path.
...
```bash
SKILL_MD_PATH="<absolute path of this loaded SKILL.md>"
...
Read [references/visual-workers.md](references/visual-workers.md) when generation is delegated to lightweight visual workers or subagents.
