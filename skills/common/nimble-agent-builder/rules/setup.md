...
| Host | Best path |
|---|---|
| Any the assistant product (opencode, the assistant Cowork, claude.ai) | **Plugin install** — `/plugin install nimble`. Auto-registers MCP as a Connector. OAuth on first use. |
...
For the full setup flow (API-key generation, permanent storage in `~/.config/opencode/settings.json`,
