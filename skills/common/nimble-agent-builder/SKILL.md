...
description: 'A building experience: create, test, validate, refine, and publish extraction workflows based on existing or new Nimble agents. For users who want to invest.'
license: MIT
allowed-tools:
- Bash
- Task
...
```bash
opencode mcp list 2>/dev/null | grep -q "nimble" && echo "MCP: connected" || echo "MCP: not connected"
```
...
- Any the assistant product (Code, Cowork, claude.ai) → `/plugin install nimble` (one command, auto-registers MCP as a Connector, OAuth handles auth).
